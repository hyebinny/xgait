...
def parse_args():
    p = argparse.ArgumentParser(description="Test GNU alignment anomaly classifier")
    p.add_argument("--cfg_pth", type=str, required=True)
    p.add_argument("--ckpt_pth", type=str, required=True)
    return p.parse_args()
...
if __name__ == "__main__":
    if any(a.startswith("--cfg_pth") for a in sys.argv[1:]):
        args = parse_args()
        main(args.cfg_pth, args.ckpt_pth)
    else:
        cfg_path = sys.argv[1]
