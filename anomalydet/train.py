...
from util.model import TimmAlignClassifier
...
def main(cfg_path: str):
    ...
    if bool(batch_sampler_cfg["enabled"]):
        ...
        bsampler = PKBatchSampler(train_ds.class_to_indices, n_class=n_class, n_samples=n_samples, drop_last=True)
        train_loader = DataLoader(train_ds, batch_sampler=bsampler, num_workers=num_workers, pin_memory=True)
        ...
        train_loader = DataLoader(
            train_ds, batch_size=effective_bs, shuffle=True,
            num_workers=num_workers, pin_memory=True, drop_last=True
        )
        bs_desc = f"{effective_bs}"

    # eval/test loaders (1번만 생성)
    train_eval_loader = DataLoader(train_ds, batch_size=64, shuffle=False, num_workers=num_workers, pin_memory=True)
    test_loader = DataLoader(test_ds, batch_size=64, shuffle=False, num_workers=num_workers, pin_memory=True)

    # model
    mcfg = cfg["model"]
    model = TimmAlignClassifier(
    ...
    # loss cfg
    lcfg = cfg["loss"]
    use_ce_ = bool(lcfg["use_ce"])
    ce_w = float(lcfg["ce_weight"])
    use_tri_ = bool(lcfg["use_triplet"])
    ...
    logger.info(f"Loss: CE({use_ce_}, w={ce_w}) + Triplet({use_tri_}, w={tri_w}, margin={tri_margin}, mining={tri_mining})")
    ...
    for epoch in range(1, epochs + 1):
        ...
        running_total, running_ce, running_tri = 0.0, 0.0, 0.0
        steps = 0
        ...
        for x, y, _path in train_loader:
            ...
            with autocast(enabled=use_fp16):
                ...
                if use_ce_:
                    ce_val = ce_loss(logits, y)

                if use_tri_:
                    if tri_mining == "hard":
                        tri_val = triplet_loss_hard(emb, y, tri_margin)
                    else:
                        tri_val = triplet_loss_all(emb, y, tri_margin)
        ...
        if steps == 0:
            raise RuntimeError("No training steps executed. Check sampler/batch_size settings.")

        avg_total = running_total / steps
        avg_ce = (running_ce / steps) if use_ce_ else 0.0
        avg_tri = (running_tri / steps) if use_tri_ else 0.0
        loss_str = build_loss_string(use_ce_, use_tri_, avg_ce, avg_tri, avg_total)
        ...
        if (epoch % eval_freq) == 0:
            ...
            if (epoch % log_epoch) == 0:
                ep_time = ep_timer.elapsed()
                logger.info(f"[Epoch {epoch:04d}/{epochs}] {loss_str} | epoch_time={ep_time:.1f}s")
...
def parse_args():
    p = argparse.ArgumentParser(description="Train GNU alignment anomaly classifier")
    p.add_argument("--cfg_pth", type=str, required=True)
    return p.parse_args()
